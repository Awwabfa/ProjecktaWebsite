...
function refresh(){
const sensor = 'http://localhost:4567/getLastestSensorData';
fetch(sensor)
.then(response => {
  if(!response.ok){
    throw new Error('Network response was not ok');
  ...
  document.getElementById("humid").innerHTML = data[2];
  document.getElementById("mois").innerHTML = data[3];
  document.getElementById("brigh").innerHTML = data[4];
})
.catch(error => {
  console.error('Error: ', error);
  window.alert("not connected to the server");
